...
| 2 | Production deploy/smoke closeout for `/saved` milestone | 1 / 7 feature areas upgraded; Feature Milestone 1 released | deploy pass (`npm run deploy`; preview `https://ec240d6e.glearning.pages.dev`); production smoke pass on preview and `https://glearning.pages.dev` | Both domains returned 200 for `/`, `/saved`, `/saved/`, `/games/wuwa`, `/games/cyberpunk`, `/audio/manifest.json`; `/api/main-quests` returned 56 quests; `/api/quest?...Utterance...&zhUrl=auto` returned paired=284, audioCount=220, lines=314. |
